...
import { loadBrandCartFromSession, loadBrandDeliveryOptions } from '@/contexts/BrandCartContext';
import { BrandCartItem, BrandDeliveryOptions } from '@/types';
...
type ShippingType = 'domestic' | 'international' | 'pickup';
...
function CheckoutContent() {
  ...
  const [deliveryOptions, setDeliveryOptions] = useState<BrandDeliveryOptions | null>(null);

  // Load brand cart and delivery options from sessionStorage
  useEffect(() => {
    if (!brandId) return;
    const loadedItems = loadBrandCartFromSession(brandId);
    setItems(loadedItems);

    const options = loadBrandDeliveryOptions(brandId);
    setDeliveryOptions(options);
    // Auto-select first available delivery option
    if (options) {
      if (options.domestic?.enabled) setShippingType('domestic');
      else if (options.international?.enabled) setShippingType('international');
      else if (options.pickup?.enabled) setShippingType('pickup');
    }
  }, [brandId]);
  ...
  const shipping = useMemo(() => {
    ...
    if (shippingType === 'domestic') return deliveryOptions.domestic?.price ?? 0;
    if (shippingType === 'international') return deliveryOptions.international?.price ?? 0;
    if (shippingType === 'pickup') return deliveryOptions.pickup?.price ?? 0;
    return 0;
  }, [shippingType, deliveryOptions]);
  ...
  const createOrder = useCallback(async () => {
    try {
      ...
      if (shippingType === 'pickup') {
        ...
        shippingStreet = deliveryOptions?.pickup?.address || '';
        shippingCity = '';
        shippingState = '';
        shippingZipCode = '';
        shippingCountry = 'KR';
        ...
        customerEmail = shippingInfo.email;
        customerPhone = shippingInfo.phone;
        shippingStreet = `${shippingInfo.address} ${shippingInfo.addressDetail}`.trim();
        shippingCity = '';
        shippingState = '';
        shippingZipCode = shippingInfo.zipCode;
        shippingCountry = 'KR';
      } else {
        customerName = internationalShippingInfo.name;
        customerEmail = internationalShippingInfo.email;
        customerPhone = internationalShippingInfo.phone;
        shippingStreet = `${internationalShippingInfo.addressLine1} ${internationalShippingInfo.addressLine2}`.trim();
        shippingCity = internationalShippingInfo.province;
        shippingState = internationalShippingInfo.state;
        shippingZipCode = internationalShippingInfo.postalCode;
        shippingCountry = internationalShippingInfo.country;
      }
  ...
  }, [orderId, orderName, total, paymentMethod, shippingType, shipping, shippingInfo, internationalShippingInfo, deliveryOptions, items]);

  // No brandId — invalid access
  if (!brandId) {
    return (
      <div className="py-12 sm:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">잘못된 접근입니다</h1>
          <p className="text-sm sm:text-base text-gray-600 mb-6 sm:mb-8">브랜드 페이지에서 상품을 담아주세요.</p>
          <Link href="/">
    ...
    return (
      <div className="py-12 sm:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">장바구니가 비어있습니다</h1>
          <p className="text-sm sm:text-base text-gray-600 mb-6 sm:mb-8">결제하기 전에 상품을 담아주세요.</p>
          <Link href="/">
            <Button size="lg">홈으로</Button>
          </Link>
        </div>
  ...
  // Count enabled delivery options for grid layout
  const enabledOptions = [
    deliveryOptions?.domestic?.enabled,
    deliveryOptions?.international?.enabled,
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-center mb-4 sm:mb-8">
          <div className="flex items-center">
            <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-sm sm:text-base font-medium ${step === 'info' ? 'bg-indigo-600 text-white' : 'bg-indigo-100 text-indigo-600'}`}>
              1
            </div>
            <span className="ml-1.5 sm:ml-2 text-sm sm:text-base font-medium text-gray-900">배송정보</span>
          </div>
          <div className="w-8 sm:w-16 h-0.5 bg-gray-200 mx-2 sm:mx-4" />
          <div className="flex items-center">
            <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-sm sm:text-base font-medium ${step === 'payment' ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-500'}`}>
              2
            </div>
            <span className={`ml-1.5 sm:ml-2 text-sm sm:text-base font-medium ${step === 'payment' ? 'text-gray-900' : 'text-gray-500'}`}>결제</span>
          </div>
        </div>
        ...
        <div className="lg:grid lg:grid-cols-12 lg:gap-8">
          ...
          <div className="lg:col-span-7">
              ...
              <form onSubmit={handleSubmitInfo} noValidate>
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6 mb-4 sm:mb-6">
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    <Input
                      label={shippingType === 'international' ? 'Name' : '이름'}
                      value={shippingType === 'international' ? internationalShippingInfo.name : shippingInfo.name}
                      onChange={(e) => {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                      <Input
                        label={shippingType === 'international' ? 'Email' : '이메일'}
                        type="email"
                        value={shippingType === 'international' ? internationalShippingInfo.email : shippingInfo.email}
                        onChange={(e) => {
                          if (shippingType === 'international') {
                      ...
                      />
                      <Input
                        label={shippingType === 'international' ? 'Phone' : '연락처'}
                        type="tel"
                        placeholder={shippingType === 'international' ? '12345678900' : '01012341234'}
                        value={shippingType === 'international' ? internationalShippingInfo.phone : shippingInfo.phone}
                        onChange={(e) => {
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6 mb-4 sm:mb-6">
                  <h2 className="text-base sm:text-lg font-semibold text-gray-900 mb-3 sm:mb-4">배송 방법</h2>
                  <div className={`grid gap-2 sm:gap-3 mb-4 sm:mb-6 ${enabledOptions === 3 ? 'grid-cols-3' : enabledOptions === 2 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                    {deliveryOptions?.domestic?.enabled && (
                      <label
                      ...
                      >
                        <input
                          ...
                          className="sr-only"
                        />
                        <span className="text-sm sm:text-base font-medium text-gray-900">국내배송</span>
                        <span className="text-xs sm:text-sm text-gray-500 mt-0.5 sm:mt-1">
                          {deliveryOptions.domestic.price === 0 ? '무료' : `${deliveryOptions.domestic.price.toLocaleString()}원`}
                        </span>
                      </label>
                    )}
                    {deliveryOptions?.international?.enabled && (
                      ...
                      >
                        <input
                          ...
                          className="sr-only"
                        />
                        <span className="text-sm sm:text-base font-medium text-gray-900">해외배송</span>
                        <span className="text-xs sm:text-sm text-gray-500 mt-0.5 sm:mt-1">
                          {deliveryOptions.international.price === 0 ? '무료' : `${deliveryOptions.international.price.toLocaleString()}원`}
                        </span>
                      </label>
                    )}
                    {deliveryOptions?.pickup?.enabled && (
                      ...
                      >
                        <input
                          ...
                          className="sr-only"
                        />
                        <span className="text-sm sm:text-base font-medium text-gray-900">현장수령</span>
                        <span className="text-xs sm:text-sm text-gray-500 mt-0.5 sm:mt-1">
                          {deliveryOptions.pickup.price === 0 ? '무료' : `${deliveryOptions.pickup.price.toLocaleString()}원`}
                        </span>
                      </label>
                    )}
                  </div>
                  ...
                  {shippingType === 'pickup' ? (
                    <>
                      <h2 className="text-base sm:text-lg font-semibold text-gray-900 mb-3 sm:mb-4">수령 장소</h2>
                      <div className="bg-gray-50 rounded-lg p-3 sm:p-4">
                        <div className="flex items-start gap-2">
                          ...
                          <p className="text-sm text-gray-700">{deliveryOptions?.pickup?.address || '수령 장소 미정'}</p>
                        </div>
                      </div>
                    </>
                  ) : shippingType === 'domestic' ? (
                    <>
                      <h2 className="text-base sm:text-lg font-semibold text-gray-900 mb-3 sm:mb-4">배송지 정보</h2>
                      <div className="space-y-3 sm:space-y-4">
                        <div className="relative">
                          ...
                          <div className="flex gap-2">
                            <input
                              ...
                              className="flex-1 px-3 sm:px-4 py-2 sm:py-2.5 text-sm sm:text-base text-gray-900 bg-white border border-gray-300 rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent placeholder:text-gray-400"
                            />
                            <Button
                              type="button"
                              variant="outline"
                          ...
                          {showAddressResults && addressResults.length > 0 && (
                            <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                              {addressResults.map((juso, index) => (
                                ...
                                >
                                  <p className="text-xs sm:text-sm font-medium text-gray-900">{juso.roadAddr}</p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                          <div>
                            ...
                            <select
                              ...
                              required
                              className="w-full px-3 sm:px-4 py-2 sm:py-2.5 text-sm sm:text-base text-gray-900 bg-white border border-gray-300 rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                            >
            ...
            {step === 'payment' && (
              <div>
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6 mb-4 sm:mb-6">
                  <h2 className="text-base sm:text-lg font-semibold text-gray-900 mb-3 sm:mb-4">결제 수단</h2>

                  <div className="space-y-2 sm:space-y-3">
                    {/* Toss Payments */}
                    <label
                      className={`flex items-center p-3 sm:p-4 border-2 rounded-xl cursor-pointer transition-all ${paymentMethod === 'toss' ? 'border-indigo-600 bg-indigo-50' : 'border-gray-200 hover:border-gray-300'}`}
                    >
                      <input
                        type="radio"
                        name="paymentMethod"
                      ...
                      <div className="flex-1">
                        <p className="text-sm sm:text-base font-medium text-gray-900">간편결제</p>
                        <p className="text-xs sm:text-sm text-gray-500">빠르고 안전하게 결제</p>
                      </div>
                      {paymentMethod === 'toss' && (
                    ...
                    {/* PayPal */}
                    <label
                      className={`flex items-center p-3 sm:p-4 border-2 rounded-xl cursor-pointer transition-all ${paymentMethod === 'paypal' ? 'border-indigo-600 bg-indigo-50' : 'border-gray-200 hover:border-gray-300'}`}
                    >
                      <input
                        type="radio"
                        name="paymentMethod"
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs sm:text-sm text-gray-500">페이팔로 안전하게 결제</p>
                      </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6 mb-4 sm:mb-6">
                  {paymentMethod === 'toss' ? (
                    <div>
                      <h3 className="text-sm sm:text-base font-medium text-gray-900 mb-3 sm:mb-4">토스페이먼츠</h3>
                      {baseUrl ? (
                        <TossPaymentWidget
                          amount={total}
                          orderId={orderId}
                          orderName={orderName}
                          customerEmail={shippingType === 'international' ? internationalShippingInfo.email : shippingInfo.email}
                          customerName={shippingType === 'international' ? internationalShippingInfo.name : shippingInfo.name}
                          customerMobilePhone={shippingType === 'international' ? internationalShippingInfo.phone : shippingInfo.phone}
                          successUrl={`${baseUrl}/checkout/success?brandId=${brandId}`}
                        ...
                        <div className="flex items-center justify-center py-6 sm:py-8">
                          ...
                          <span className="ml-2 sm:ml-3 text-sm sm:text-base text-gray-600">결제 위젯 로딩 중...</span>
                        </div>
                      )}
                    </div>
                  ) : (
                    <div>
                      <h3 className="text-sm sm:text-base font-medium text-gray-900 mb-3 sm:mb-4">PayPal</h3>
                      <PayPalPaymentButton
                        amount={total}
                        orderId={orderId}
                        orderName={orderName}
                        ...
                        onSuccess={(details) => {
                          ...
                          router.push(`/checkout/success?paymentMethod=paypal&orderId=${orderId}&brandId=${brandId}`);
                        }}
                        onError={(error) => {
          ...
          <div className="lg:col-span-5 mt-6 lg:mt-0">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6 sticky top-24">
              <h2 className="text-base sm:text-lg font-semibold text-gray-900 mb-3 sm:mb-4">주문 요약</h2>
              ...
              <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-6 max-h-48 sm:max-h-64 overflow-y-auto">
                {items.map(item => {
                  ...
                  return (
                    <div key={itemKey} className="flex gap-2 sm:gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs sm:text-sm text-gray-500">수량: {item.quantity}</p>
                      </div>
              ...
              <div className="border-t border-gray-200 pt-3 sm:pt-4 space-y-2 sm:space-y-3">
                ...
                <div className="border-t border-gray-200 pt-2 sm:pt-3 flex justify-between font-semibold text-gray-900 text-sm sm:text-lg">
                  <span>총 결제금액</span>
              ...
              <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-gray-200">
                <div className="flex items-center justify-center gap-1.5 sm:gap-2 text-gray-500">
                  ...
                  <span className="text-xs sm:text-sm">SSL 보안 암호화</span>
                </div>
              </div>
            </div>
          </div>
